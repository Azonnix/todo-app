...
func main() {
	...
	if err != nil {
		logrus.Fatalf("faild initialize db: %s", err.Error())
	}
	...
	if err := srv.Shotdown(context.Background()); err != nil {
		logrus.Errorf("error occured server is shutting down: %s", err.Error())
	}

	if err := db.Close(); err != nil {
		logrus.Errorf("error occured od db connection close: %s", err.Error())
	}
}
